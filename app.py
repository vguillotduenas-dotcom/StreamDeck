...
class Video(db.Model):
    ...
    genre = db.Column(db.String(50), default='Autre')
...
def admin():
    ...
    if request.method == 'POST':
        if 'add_video' in request.form:
            v = Video(nom=request.form['nom'], img=request.form['img'], lien=request.form['lien'], type=request.form['type'], genre=request.form['genre'])
            db.session.add(v)
